...
class VPSBackupUtils:
    def __init__(self, backup_dest: PathLikeStr):
        ...
        self.today_backup_path = os.path.join(self.backup_dest, datetime)
        self.logger.info(f'today backup path: {self.today_backup_path}')
        os.makedirs(self.today_backup_path, exist_ok=True)
    ...
    def mysqldump_backup(self,
        ...
        def dump_and_gzip(filename: str, db: str = '', all_databases: bool = True):
            output_filename = os.path.join(self.backup_dest, filename)
    ...
    def tar_backup(self,
        ...
        need `tar` in PATH
        """
        tar(src_folder, os.path.join(self.today_backup_path, f'{backup_prefix}.tar'), gzipped=gzipped)
        self.logger.info(f'{backup_prefix}: {src_folder} data backup finished')

    def remove_old_backup(self, days_to_keep: int = 7):
        """
    ...
    def rsync_backup(self,
        ...
        """
        rsync(local_path=self.today_backup_path, host=host, user=user, remote_path=remote_backup_path, port=port,
